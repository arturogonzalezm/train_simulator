...
def time_sequence_decorator(func):
    def wrapper(time_sequence, *args, **kwargs):
        if not isinstance(time_sequence, list):
            return func(time_sequence, *args, **kwargs)
        sanitized_time_sequence = convert_to_datetime(time_sequence)
        return func(sanitized_time_sequence, *args, **kwargs)
...
def verify_time_format(time_str):
    ...
    if matched:
        if 0 <= int(matched.group(1)) < 24 and 0 <= int(matched.group(2)) < 60:
            ...
            logger.info('Hour should be within [0, 24); Minute should be within [0, 60)')
            return False
    else:
        return False
...
def verify_duration(time_str):
    ...
    if duration:
        ...
        if 0 <= minutes < 60 and hour >= 0 and any([hour, minutes]):
            ...
            logger.info('Minute should be within [0, 60)')
            return False
    else:
        logger.info("Invalid format, should be '%H:%M' ")
        return False
